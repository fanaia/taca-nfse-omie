...
} = require("../services/taca/workflow");
...
async function dashboard() {
  ...
  const processingStatuses = ["RECEIVED", "VALIDATED", "CUSTOMER_SYNCED", "SERVICE_ORDER_CREATED", "INVOICE_PROCESSING"];
  const [emissionsToday, issued, processing, errors, orderCallbacksPending, reversalCallbacksPending, reversalsPending, avgRows, lastHour, oldestPending] = await Promise.all([
    ...
    PedidoNfse.aggregate([
      { $match: { invoiceConfirmedAt: { $ne: null }, receivedAt: { $ne: null } } },
      { $project: { duration: { $subtract: ["$invoiceConfirmedAt", "$receivedAt"] } } },
...
async function configOptions() {
  const { ServicoOmie, CategoriaOmie, ContaCorrenteOmie, CondicaoPagamentoOmie, CidadeOmie } = models();
  ...
  return {
    ...
    services: services.map((item) => option(item._id, item.codigoServicoOmie, `${item.codigo ? `${item.codigo} — ` : ""}${item.nome}`, { categoryCode: item.codigoCategoriaOmie || "" })),
    categories: categories.map((item) => option(item._id, item.codigoCategoriaOmie, `${item.codigoCategoriaOmie} — ${item.nome}`)),
    currentAccounts: currentAccounts.map((item) => option(item._id, item.codigoContaCorrenteOmie, `${item.nome} (${item.codigoContaCorrenteOmie})`)),
    paymentTerms: paymentTerms.map((item) => option(item._id, item.codigoCondicaoPagamento, `${item.codigoCondicaoPagamento} — ${item.nome}`)),
    cities: cities.map((item) => option(item._id, item.codigoCidadeOmie, `${item.nome}${item.uf ? ` (${item.uf})` : ""}`, { ibgeCode: item.codigoIbge || "", state: item.uf || "" })),
  };
}
