...
test("OpenAPI documents authentication and both public platform operations", () => {
  ...
  assert.ok(openapi.paths["/auth/autenticar"]?.post);
  assert.deepEqual(openapi.paths["/auth/autenticar"].post.security, [{ basicAuth: [] }]);
...
test("Swagger UI and OpenAPI JSON are explicitly public routes", () => {
